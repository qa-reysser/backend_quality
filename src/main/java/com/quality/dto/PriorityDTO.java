...
public class PriorityDTO {
    ...
    @Schema(description = "Nombre de la prioridad (único)", example = "Alta", required = true, minLength = 3, maxLength = 70)
    private String name;

    @NotNull(message = "Description is required")
    @NotEmpty(message = "Description cannot be empty")
    @Size(min = 3, max = 70, message = "Description must be between 3 and 70 characters")
    @Schema(description = "Descripción de la prioridad", example = "Tareas de alta prioridad", required = true, minLength = 3, maxLength = 70)
